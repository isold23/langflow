...
export const processKnowledges = (DbData: KnowledgeType[], skipUpdate = true) => {
  let savedComponents: { [key: string]: APIClassType } = {};
  DbData.forEach((knowledge: KnowledgeType) => {
    try {
      if (!knowledge.data) {
        return;
      }
      if (knowledge.data && knowledge.is_component) {
        (knowledge.data.nodes[0].data as NodeDataType).node!.display_name =
        knowledge.name;
        savedComponents[
          createRandomKey((knowledge.data.nodes[0].data as NodeDataType).type, uid())
        ] = cloneDeep((knowledge.data.nodes[0].data as NodeDataType).node!);
        return;
      }
      if (!skipUpdate) processDataFromKnowledge(knowledge, false);
    } catch (e) {
      console.log(e);
    }
  });
  return { data: savedComponents, knowledges: DbData };
};

export const processDataFromKnowledge = (knowledge: KnowledgeType, refreshIds = true) => {
  let data = knowledge?.data ? knowledge.data : null;
  if (data) {
    processKnowledgeEdges(knowledge);
...
export function processKnowledgeEdges(knowledge: KnowledgeType) {
  if (!knowledge.data || !knowledge.data.edges) return;
  if (checkOldEdgesHandles(knowledge.data.edges)) {
    const newEdges = updateEdgesHandleIds(knowledge.data);
    knowledge.data.edges = newEdges;
  }
  //update edges colors
  knowledge.data.edges.forEach((edge) => {
    edge.className = "";
...
export function createKnowledgeComponent(
  ...
  const knowledgeNode: KnowledgeType = {
    data: {
      edges: [],
      nodes: [
        {
...
export const createNewKnowledge = (
  knowledgeData: ReactFlowJsonObject,
  knowledge: KnowledgeType
) => {
  return {
    description: knowledge?.description ?? getRandomDescription(),
    name: knowledge?.name ? knowledge.name : "Untitled document",
    data: knowledgeData,
    id: "",
    is_component: knowledge?.is_component ?? false,
  };
};
